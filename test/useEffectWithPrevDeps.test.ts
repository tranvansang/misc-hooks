...
describe('useEffectWithPrevDeps', () => {
	it('should call effect with empty array on mount', () => {
		...
		renderHook(() => useEffectWithPrevDeps(effect, ['dep1', 'dep2'] as const))
	...
	it('should call effect with previous deps on update', () => {
		...
		const {rerender} = renderHook(
			({deps}) => useEffectWithPrevDeps(effect, deps),
	...
	it('should work with empty deps array', () => {
		...
		const {rerender} = renderHook(() => 
			useEffectWithPrevDeps(effect, [] as const)
		)
	...
	it('should handle cleanup function returning undefined', () => {
		...
		const {unmount} = renderHook(() => 
			useEffectWithPrevDeps(effect, [1] as const)
		)
